...
export const loginUser = createAsyncThunk('auth/login', async (credentials, { rejectWithValue }) => {
  try {
    ...
    const formattedPermissions = {
      ...
      isAdmin: user?.is_admin || user?.role === 'admin' || permissions?.isAdmin || false,
    };
...
export const hasPermission = (state, moduleName, requiredAction = null) => {
  ...
  const isAdmin = user?.is_admin || user?.role === 'admin' || permissions?.isAdmin;
  ...
  const hasAction = permissions?.functionalities?.some((func) => {
    ...
    return funcName.includes(requiredAction.toLowerCase()) && 
           (func.module_id ? true : true); // Optionally check module_id match
  });
...
const authSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(loginUser.fulfilled, (state, action) => {
      ...
      state.user = user;
      state.token = token;
      state.permissions = permissions || { modules: [], functionalities: [], isAdmin: false };
    });
    ...
    builder.addCase(refreshPermissions.fulfilled, (state, action) => {
      state.loading = false;
      state.permissions = action.payload || { modules: [], functionalities: [], isAdmin: false };
    });
