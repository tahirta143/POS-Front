...
function StatCard({
  ...
  return (
    <div className="group bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 p-4 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 relative overflow-hidden">
      ...
      <p className="text-2xl font-bold text-slate-800 dark:text-slate-100 leading-none mb-2 transition-colors">
        {isCurrency ? formatCurrency(value) : value.toLocaleString()}
      </p>
      <div className="flex items-center gap-1">
        <div className="flex items-center gap-0.5 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 text-[10px] font-bold px-1.5 py-0.5 rounded-full transition-colors">
...
function StatusBadge({ status }) {
  ...
  return (
    <span
      className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold ${map[status] ?? "bg-slate-50 text-slate-500"}`}
    >
      {status}
    </span>
  );
...
function ChartTooltip({ active, payload, label }) {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-lg px-3 py-2.5 text-xs transition-colors">
...
export default function Dashboard() {
    ...
    try {
      const [customersRes, productsRes, staffRes, bookingsRes, revenueRes] =
        await Promise.all([
          axiosInstance
            .get("/customers/count")
            .catch(() => ({ data: { totalCustomers: 0 } })),
          axiosInstance
            .get("/item-details/count")
            .catch(() => ({ data: { count: 16 } })),
          axiosInstance
            .get("/staff/count")
            .catch(() => ({ data: { count: 0 } })),
          ...
          axiosInstance
            .get("/sale-invoices/revenue")
            .catch(() => ({ data: { revenue: 11650 } })),
        ]);
      ...
      setStats({
        totalCustomers: {
          value: customersRes.data?.totalCustomers ?? 0,
          change: 12,
        },
  ...
  const totalOrders =
    orderStatus.pending + orderStatus.completed + orderStatus.rejected;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div className="lg:col-span-2">
            <SectionCard
              ...
              action={
                <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-xl gap-0.5 transition-colors">
          ...
          <div className="lg:col-span-1">
            <SectionCard
              ...
              subtitle="Status breakdown of advance orders"
            >
              {loading || !isMounted ? (
                <Spinner />
              ) : totalOrders === 0 ? (
                  ...
                  <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
                    {recentBookings.map((b) => (
                      ...
                      >
                        ...
                        <td className="py-2.5 text-[12px] text-slate-500 dark:text-slate-400">
                          {formatDate(b.booking_date)}
                        </td>
                        <td className="py-2.5 text-[12px] font-semibold text-slate-700 dark:text-slate-200">
                          {formatCurrency(b.payable || 0)}
                        </td>
                ...
                <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {recentSales.map((s) => (
                      ...
                      >
                        <td className="py-2.5 pl-1">
                          <span className="text-[11px] font-mono font-semibold text-teal-600 bg-teal-50 dark:bg-slate-800 dark:text-teal-400 px-1.5 py-0.5 rounded-md">
                            #{s.id}
                          </span>
                        </td>
                        <td className="py-2.5">
                          <p className="text-[13px] font-medium text-slate-800 dark:text-slate-50">
