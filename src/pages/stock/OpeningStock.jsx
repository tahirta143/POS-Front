...
import { MdLock, MdRefresh, MdSearch } from "react-icons/md";
...
function MetricCard({ title, value, valueColor }) {
  return (
    <div className="flex flex-col rounded-2xl border border-slate-200/90 bg-white/90 p-4 shadow-sm ring-1 ring-white/70 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-md dark:border-slate-800 dark:bg-slate-900/90 dark:ring-slate-800/80">
      <p className="text-[11px] font-bold text-slate-400 uppercase tracking-[0.2em] dark:text-teal-400">
        {title}
      </p>
      <p
        className={`mt-2 text-2xl font-bold ${valueColor} dark:text-slate-100`}
      >
        {value}
      </p>
    </div>
...
export default function OpeningStockPage() {
  ...
  async function fetchData() {
    ...
    try {
      ...
      if (itmRes?.data) {
        nextItems = Array.isArray(itmRes.data)
          ? itmRes.data
          : itmRes.data.data || [];
      }
      if (catRes?.data) {
        nextCategories = Array.isArray(catRes.data)
          ? catRes.data
          : catRes.data.data || [];
      }
      if (supRes?.data) {
        nextSuppliers = Array.isArray(supRes.data)
          ? supRes.data
          : supRes.data.data || [];
      }
  ...
  async function commitStockChange(itemId, newStockRaw) {
    ...
    try {
      ...
      setItems((prev) =>
        prev.map((i) => (i.id === itemId ? { ...i, stock: lastSaved } : i)),
      );
  ...
  const filteredItems = useMemo(() => {
    return items.filter((item) => {
      ...
      const matchesCat =
        !categoryFilter ||
        String(item.item_category_id) === String(categoryFilter);
      ...
      const matchesDate =
        !dateFilter || (item.created_at || "").includes(dateFilter);
  ...
  const purchaseValue = items.reduce(
    (sum, item) =>
      sum + (Number(item.stock) || 0) * (Number(item.purchase_price) || 0),
    0,
  );
  const saleValue = items.reduce(
    (sum, item) =>
      sum + (Number(item.stock) || 0) * (Number(item.sale_price) || 0),
    0,
  );
      ...
      <div className="mx-auto max-w-[1400px] space-y-6">
        ...
        <div className="flex flex-col items-start justify-between gap-4 px-5 py-5 backdrop-blur sm:flex-row sm:items-center">
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">
              View and manage initial stock quantities
            </p>
          </div>
          <button
            onClick={fetchData}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <MetricCard
            ...
            value={`Rs ${purchaseValue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`}
            valueColor="text-teal-600"
          />
          <MetricCard
            title="Sale Value"
            value={`Rs ${saleValue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`}
            valueColor="text-teal-600"
          />
        </div>
        ...
        <Card className="border-l-[6px] border-l-teal-500 p-0 overflow-hidden">
          <div className="flex items-center gap-2 px-5 py-4 border-b border-slate-100 dark:border-teal-500/50 dark:bg-teal-600 transition-colors">
            ...
            <h2 className="text-[12px] font-bold uppercase tracking-wide text-slate-800 dark:text-white">
              Filter Stock
            </h2>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-end p-5">
            <div>
              <label className="block text-[11px] font-bold text-slate-500 dark:text-teal-400 uppercase tracking-wider mb-1.5">
                Search
              </label>
              <div className="relative">
                ...
                <input
                  ...
                  className="h-8 w-full pl-8 pr-3 rounded-lg border border-slate-200 bg-slate-50 text-[12px] outline-none focus:border-teal-400 focus:bg-white transition"
            ...
            <div>
              ...
              <select
                ...
                className="h-8 w-full rounded-lg border border-slate-200 bg-slate-50 text-slate-800 dark:text-slate-200 px-2.5 text-[12px] outline-none transition focus:border-teal-400 focus:bg-white"
              >
            ...
            <div>
              ...
              >
                ...
                {suppliers.length > 0
                  ...
                  : [
                      ...new Set(items.map((i) => i.supplier).filter(Boolean)),
                    ].map((s, idx) => (
          ...
          <div className="flex justify-between items-center mt-5 pt-4 border-t border-slate-100">
            <p className="text-[12px] text-slate-500">
              ...
              of <strong className="text-slate-800">{items.length}</strong>{" "}
              items
            </p>
            <button
              onClick={() => {
              ...
              className="inline-flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3 py-1.5 text-[12px] font-semibold text-slate-600 hover:bg-slate-50 transition shadow-sm"
            >
              <MdRefresh className="h-3.5 w-3.5" /> Reset
            </button>
          </div>
        </Card>
              ...
              <tbody className="divide-y divide-slate-100 bg-white">
                {filteredItems.length === 0 ? (
                  ...
                  filteredItems.map((item, idx) => (
                    <tr
                      key={item.id || idx}
                      className="text-[12px] transition hover:bg-slate-50/80 dark:hover:bg-slate-800/40"
                    >
                      ...
                      <td className="px-4 py-3 text-right font-semibold text-teal-600">
                        Rs {Number(item.sale_price || 0).toFixed(2)}
                      </td>
                      <td className="px-4 py-3 text-center">
                        <div className="relative inline-block">
                          <input
                            ...
                            onBlur={(e) =>
                              commitStockChange(item.id, e.target.value)
                            }
