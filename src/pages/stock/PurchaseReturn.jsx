...
import { ActionButton, Card, Field, PageShell, SectionHeader, TableState, StatusChip } from '../../components/layout/PageShell.jsx'
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-1">
                          ...
                          {selectedPurchase && (
                            <div className="flex flex-col justify-center rounded-lg bg-teal-50 px-3 py-1 border border-teal-100">
                              ...
                              <p className="text-sm font-black text-teal-700 truncate font-mono">PKR {totalReturnValue.toLocaleString()}</p>
                            </div>
                          )}
                        </div>
                      </SectionCard>
                    ...
                    <SectionCard title="Entity Details">
                      <div className="space-y-2 p-1">
                        <div className="flex items-center gap-2 rounded-lg border border-slate-100 bg-slate-50 px-2 py-1.5">
                          <div className="flex h-7 w-7 items-center justify-center rounded-md bg-white text-slate-400 border border-slate-100"><MdBusiness className="h-4 w-4" /></div>
                          <div className="min-w-0">
                            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest leading-none">Supplier</p>
                            <p className="text-[11px] font-bold text-slate-700 truncate">{selectedSupplierName}</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-2 rounded-lg border border-slate-100 bg-slate-50 px-2 py-1.5">
                          <div className="flex h-7 w-7 items-center justify-center rounded-md bg-white text-slate-400 border border-slate-100"><MdPriceCheck className="h-4 w-4" /></div>
                          <div className="min-w-0">
                            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest leading-none">Original Ref</p>
                            <p className="text-[11px] font-bold text-slate-700 truncate">{selectedPurchase?.grn_no || selectedPurchase?.invoice_no || '-'}</p>
                          </div>
                        </div>
                      </div>
                    </SectionCard>
                  ...
                  {selectedPurchase && (
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mt-4 space-y-4">
                          ...
                          <table className="w-full text-left text-[11px]">
                            <thead className="bg-slate-50 text-[10px] font-bold uppercase tracking-wider text-slate-500">
                              <tr>
                                ...
                                <th className="px-4 py-2 text-right">Inward Qty</th>
                              ...
                              {returnItems.map(item => {
                                ...
                                return (
                                  <tr key={itemId} className="hover:bg-slate-50/50 transition-colors">
                                    <td className="px-4 py-2 font-semibold text-slate-700">{item.item_name}</td>
                                    <td className="px-4 py-2 text-right text-slate-500">{inwardQty}</td>
                                    <td className="px-4 py-2 text-right font-mono">PKR {rate.toLocaleString()}</td>
                                    ...
                                    <td className="px-4 py-2 text-right font-black text-teal-700">PKR {returnTotal.toLocaleString()}</td>
                                  </tr>
                                );
                              })}
                            </tbody>
                      ...
                      <div className="flex justify-end gap-3 pt-2">
                        <button type="button" onClick={resetForm} className="rounded-xl px-5 py-2 text-xs font-semibold text-slate-500 hover:bg-slate-100 transition">Discard Changes</button>
                        <button type="submit" disabled={submitting} className="inline-flex min-w-[180px] items-center justify-center gap-2 rounded-xl bg-teal-600 px-6 py-2 text-sm font-black text-white shadow-lg shadow-teal-100 hover:bg-teal-700 transition disabled:opacity-50">
                          {submitting ? 'Recording...' : editId ? 'Update Debit Note' : 'Authorize Return'}
                        </button>
                      </div>
                    </motion.div>
                  )}
                    ...
                    <motion.tr key={record.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className={`group transition-colors hover:bg-teal-50/30 ${editId === record.id ? 'bg-teal-50/50' : ''}`}>
                      ...
                      <td className="px-5 py-4">
                        <div className="flex flex-col">
                          ...
                          <span className="text-[10px] text-slate-400">{record.return_date ? new Date(record.return_date).toLocaleDateString() : '-'}</span>
                        </div>
                      </td>
                      ...
                      <td className="px-5 py-4">
                        <div className="flex justify-end gap-2 text-right">
                          {canU && <ActionButton label="Edit" tone="teal" onClick={() => handleEdit(record)} />}
                          {canD && <ActionButton label="Delete" tone="rose" onClick={() => handleDelete(record.id)} />}
                        </div>
                      </td>
                    </motion.tr>
                  ))}
