...
export default function CustomerLedger() {
  ...
  const filteredCustomers = useMemo(() => {
    ...
    return customers.filter(
      c => c.customer_name?.toLowerCase().includes(q) || c.mobile_number?.includes(q)
    );
  ...
  const filteredLedger = useMemo(() => {
    ...
    return ledger.filter(
      t => t.description?.toLowerCase().includes(q) || t.reference?.toLowerCase().includes(q)
    );
  }, [ledger, tableSearch]);
  ...
  return (
    <PageShell title="Customer Ledger" description="Detailed transaction history for individual customers." accent="from-teal-600 via-emerald-600 to-cyan-700">
      <div className="space-y-5 max-w-6xl mx-auto">

        {/* Customer Selector */}
        <Card className="p-5 border-l-[6px] border-l-teal-500">
          <SectionHeader title="Customer Selection" description="Search and select a customer to view their statement." icon={<BookIcon className="h-5 w-5" />} />
          <div className="mt-4 relative max-w-lg">
            <div className="flex gap-2">
              <div className="relative flex-1">
                <input
                  ...
                  className="h-9 w-full rounded-md border border-slate-300 bg-white pl-9 pr-3 text-[12px] outline-none focus:border-teal-400 transition"
                />
                <svg className="absolute left-3 top-2.5 h-4 w-4 text-slate-400 pointer-events-none" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                </svg>
              </div>
              {selectedCustomer && (
                <button onClick={clearSelection} className="rounded-lg border border-slate-200 px-3 py-1.5 text-[12px] font-medium text-slate-600 hover:bg-slate-50 transition">
                  Clear
                </button>
              )}
              {selectedCustomer && (
                <button onClick={() => fetchLedger(selectedCustomer.id)} className="rounded-lg border border-teal-200 bg-teal-50 px-3 py-1.5 text-[12px] font-medium text-teal-600 hover:bg-teal-100 transition">
                  ↻ Reload
                </button>
              )}
            </div>
            ...
            {/* Dropdown */}
            {showDropdown && (
              <div className="absolute z-50 mt-1 w-full rounded-lg border border-slate-200 bg-white py-1 shadow-lg max-h-48 overflow-y-auto">
                {filteredCustomers.map(c => (
                  <button key={c.id} onClick={() => handleCustomerSelect(c)}
                    className="w-full px-3 py-2 text-left text-[12px] hover:bg-teal-50 flex items-center justify-between border-b last:border-0 transition">
                    <div>
                      <span className="font-semibold text-slate-800">{c.customer_name}</span>
                      <span className="ml-2 text-slate-400 text-[11px]">{c.mobile_number}</span>
                    </div>
                    <span className="text-[10px] text-slate-400">PKR {parseFloat(c.previous_balance || 0).toFixed(2)}</span>
                  </button>
                ))}
              </div>
            )}
        ...
        {selectedCustomer && !loading && ledgerData && (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {[
              { label: 'Customer',        value: selectedCustomer.customer_name, color: 'text-slate-800' },
              { label: 'Opening Balance', value: `PKR ${openingBalance.toFixed(2)}`, color: 'text-slate-700' },
              { label: 'Total Debit',     value: `PKR ${totalDebit.toLocaleString(undefined, { minimumFractionDigits: 2 })}`, color: 'text-rose-600' },
              { label: 'Net Balance',     value: `PKR ${Math.abs(closingBalance).toFixed(2)} ${closingBalance > 0 ? 'DR' : closingBalance < 0 ? 'CR' : ''}`, color: closingBalance > 0 ? 'text-rose-600' : 'text-emerald-600' },
            ].map(({ label, value, color }) => (
              <Card key={label} className="p-3">
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block">{label}</span>
                <p className={`text-[13px] font-bold truncate mt-0.5 ${color}`}>{value}</p>
              </Card>
            ))}
          </div>
        )}
        ...
        {selectedCustomer && ledgerData && (
          <Card className="p-3">
            <div className="flex items-center justify-between mb-1">
              <span className="text-[11px] font-bold text-slate-500 uppercase tracking-widest">Credit Limit Usage</span>
              <span className="text-[11px] font-bold text-slate-600">{creditUsedPercent.toFixed(1)}% of PKR {CREDIT_LIMIT.toLocaleString()}</span>
            </div>
            <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden">
              <div
                className={`h-full rounded-full transition-all ${creditUsedPercent > 80 ? 'bg-rose-500' : creditUsedPercent > 50 ? 'bg-amber-400' : 'bg-teal-500'}`}
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 px-5 py-4 border-b border-slate-100 bg-slate-50">
              <div>
                <h3 className="text-[14px] font-bold text-slate-800">
                  Statement — {ledgerData?.customer?.customer_name || selectedCustomer.customer_name}
                </h3>
                <p className="text-[11px] text-slate-400 mt-0.5">{filteredLedger.length} transaction{filteredLedger.length !== 1 ? 's' : ''}</p>
              </div>
              <div className="flex items-center gap-3">
                <input type="text" value={tableSearch} onChange={e => setTableSearch(e.target.value)}
                  placeholder="Filter transactions..."
                  className="h-8 w-48 rounded-lg border border-slate-200 bg-white px-3 text-[12px] outline-none focus:border-teal-400 transition" />
                <div className="text-right shrink-0">
                  ...
                  <span className={`text-base font-black font-mono ${closingBalance > 0 ? 'text-rose-600' : closingBalance < 0 ? 'text-emerald-600' : 'text-slate-500'}`}>
                    ...
                    {closingBalance !== 0 && <span className="ml-1 text-[10px]">{closingBalance > 0 ? 'DR' : 'CR'}</span>}
                  </span>
                </div>
              </div>
            </div>
              ...
              <table className="min-w-full divide-y divide-slate-100 text-left">
                    ...
                    filteredLedger.map((txn, idx) => (
                      <tr key={idx} className="hover:bg-slate-50/60 transition-colors text-sm">
                        ...
                        <td className="px-5 py-3 text-right font-semibold text-rose-600 font-mono text-[12px]">
                          {txn.debit  > 0 ? `PKR ${Number(txn.debit).toLocaleString(undefined, { minimumFractionDigits: 2 })}` : '—'}
                        </td>
                        <td className="px-5 py-3 text-right font-semibold text-emerald-600 font-mono text-[12px]">
                          {txn.credit > 0 ? `PKR ${Number(txn.credit).toLocaleString(undefined, { minimumFractionDigits: 2 })}` : '—'}
                        </td>
                        <td className={`px-5 py-3 text-right font-bold font-mono text-[12px] ${Number(txn.balance) > 0 ? 'text-rose-700' : Number(txn.balance) < 0 ? 'text-emerald-700' : 'text-slate-400'}`}>
                          PKR {Math.abs(Number(txn.balance || 0)).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                          {Number(txn.balance) !== 0 && <span className="ml-1 text-[9px]">{Number(txn.balance) > 0 ? 'DR' : 'CR'}</span>}
                        </td>
                      </tr>
                    ))
                  )}
                ...
                {!loading && filteredLedger.length > 0 && (
                  <tfoot className="bg-slate-50 border-t-2 border-slate-200">
                    <tr>
                      <td colSpan="3" className="px-5 py-3 text-[11px] font-bold uppercase text-slate-500 text-right">Totals</td>
