      ...
      <div className="space-y-5">
        ...
        <Card>
          ...
          {loading ? (
            ...
            <div className="space-y-3 lg:max-h-[22rem] lg:overflow-y-auto w-full">
              {shelves.map((shelf, index) => (
                <div key={shelf.id ?? index} className="rounded-2xl border border-slate-100 bg-slate-50/70 p-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      <div className="flex items-center gap-2">
                        <p className="text-sm font-semibold text-slate-900">{shelf.shelf_name_code}</p>
                      </div>
                      {shelf.description && <p className="mt-1 max-w-xs text-sm text-slate-500">{shelf.description}</p>}
                    </div>
