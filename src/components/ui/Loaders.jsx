...
export function Spinner({ size = "md", className = "" }) {
  ...
  return (
    <div className={`flex items-center justify-center py-6 ${className}`}>
      <div
        className={`${sizes[size] || sizes.md} border-primary-200 border-t-primary-600 rounded-full animate-spin`}
      />
    </div>
  );
}
  ...
  return (
    <div className="w-full">
      ...
      {Array.from({ length: rows }).map((_, rowIdx) => (
        <div
          key={`row-${rowIdx}`}
          className="flex py-3 border-b border-slate-50 dark:border-slate-800/50"
        >
